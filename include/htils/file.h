 ...
 *
 * @details Opens the file and defers to \ref file_size_stream().
 *
 ...
 *
 * @details Reads the stream to EOF and returns it as a \ref string, with the
 * byte count in the string's `len`, using <a
 ...
 *
 * @details Opens the file and defers to \ref read_file_from_stream().
 *
 * @pre
 ...
 *
 * @pre
 * - @c arena and @c stream must be valid and cannot be `null`.
 * - @c stream should be preferably open with "rb".
 * - @c bytes must be greater than 0.
 *
 * @param arena For allocating the contents of the file.
 * @param stream The file stream to read from.
...
/**
 * @brief Write @c bytes from @c contents to a file at @c path.
 *
 ...
 *
 * @pre
 * - @c path and @c contents must be valid, and cannot be `null`.
 * - @c bytes must be greater than 0.
 *
 * @param path The path to the file to write to.
 * @param contents The \ref string to write to the file.
